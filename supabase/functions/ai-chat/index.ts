...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof rawMessage !== "string" || rawMessage.trim().length === 0 || rawMessage.length > 4000) {
      return new Response(
        JSON.stringify({ error: "Invalid message: must be a non-empty string up to 4000 characters." }),
    ...
    let safeHistory: { role: "user" | "assistant"; content: string }[] = [];
    if (rawHistory !== undefined && rawHistory !== null) {
      ...
      for (const entry of rawHistory.slice(-MAX_HISTORY)) {
        if (
          ...
          // Silently drop invalid/system-role entries to prevent prompt injection
          continue;
        }
